                                ...
                                int32_t event_id, void* event_data)
{
    ...
    if (event_base == WIFI_EVENT && event_id == WIFI_EVENT_STA_START) {
        ...
        printf("[WiFi] MSG -> Station started. Connecting to AP...\n");
        esp_wifi_connect();
    } 
    // 2. 糟糕，连接断开了！（密码错、信号差、或者路由器重启）
    else if (event_base == WIFI_EVENT && event_id == WIFI_EVENT_STA_DISCONNECTED) {
        g_wifi_connected = false;
        printf("[WiFi] WRN -> Connection lost. Retrying (#%d)...\n", s_retry_num + 1);
    ...
    // 3. 狂喜！成功获取到了路由器的 IP 地址
        ...
        printf("[WiFi] OK -> Connected! Got IP: " IPSTR "\n", IP2STR(&event->ip_info.ip));
...
void bsp_wifi_init(void)
{
    ...
    printf("[WiFi] INIT -> Initializing Station Mode...\n");
    ...
    printf("[WiFi] OK -> Hardware driver started. Awaiting connection to \"%s\"...\n", VCU_WIFI_SSID);
}